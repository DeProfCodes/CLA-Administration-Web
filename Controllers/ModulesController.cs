    ...
    public class ModulesController : Controller
    {
        ...
        public IActionResult ContentLibraryCategories()
        {
            ...
            try
            {
                ...
                data.CategoriesTrees.OrderByDescending(c => c.ContentsCount).ToList();
        ...
        public async Task<IActionResult> ContentLibraryContent(int categoryId)
        {
            ...
            contents = contents.Where(c => c.CategoryId == categoryId).ToList();
            ...
            var contentsViewModel = new ContentLibraryContentsViewModel
            {
                ...
                CategoryTree = categoryTree.ContentLibraryParents,
                ContentLibraryContents = contents
            };
        ...
        public async Task<IActionResult> ContentLibraryContentDetails(int contentId)
        {
            ...
            var categoryTree = ModulesHelper.GetCategoryTreeStructure(ModulesMockData.AllContentLibraryCategories, contentInfo.CategoryId);

            contentInfo.TargetedModulesFullName = ModulesHelper.ConvertTargetedModulesToFullNames(contentInfo.TargetedModules);
            contentInfo.CategoryTree = categoryTree.ContentLibraryParents;

            return PartialView(AppPagesLinks.Modules.ContentLibraryContentDetailsPageLink, contentInfo);
        }

        public async Task<IActionResult> AddNewContentLibraryContent(ModuleNamesType moduleName, int categoryId, int contentId)
        {
            var contentInfo = ModulesMockData.AllContentLibraryContents.FirstOrDefault(x => x.ContentId == contentId);
            ...
            {
                categoryTreeVm = ModulesHelper.GetCategoryTreeStructure(ModulesMockData.AllContentLibraryCategories, categoryId);
            }

            categoryTreeVm.CategoryId = categoryId;
            categoryTreeVm.ContentLibraryModel = contentInfo ?? new();
            categoryTreeVm.AddOrEdit = contentInfo == null ? AddOrEditType.Add : AddOrEditType.Edit;
